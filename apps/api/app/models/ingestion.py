...
class ObservabilitySource(BaseModel):
    ...
    status: Mapped[ObservabilitySourceStatus] = mapped_column(
        Enum(ObservabilitySourceStatus), default=ObservabilitySourceStatus.UNKNOWN, nullable=False
    )
...
class HealthCheckEvent(BaseModel):
    ...
    component_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("system_components.id"), nullable=False, index=True
    )
    timestamp: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, index=True)
    status: Mapped[HealthStatus] = mapped_column(Enum(HealthStatus), nullable=False, index=True)
